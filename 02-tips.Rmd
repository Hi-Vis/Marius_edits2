...
# This fails to converge if we don't set the fix_missing=TRUE option
...
your data into R from any format designed for tabular data.
...
* `get()` and `assign()`: These allow you to look up and create variable
  ...
  programmatically create the variable name `get(paste0("model", model_num))`.
  ...
  - People often use these when they want to run 100 different versions
    of a model (there are good reasons to do this). Instead of creating
    ...
    them all in a list means it's much easier to then process and work
    with the results.